
package org.lonpe.mapstore;            

            

import org.lonpe.model.Product;
import org.lonpe.services.DBLon1;
import org.lonpe.services.DBLon0;
import java.util.Collection;
import java.util.Map;
import java.util.LinkedHashMap;

    
public class ProductMapStore extends AbstractDCMapStore<Product>{

    private DBLon1 dBLon1;
    private DBLon0 dBLon0;

    public ProductMapStore(DBLon1 dBLon1,DBLon0 dBLon0) {
        this.dBLon1 = dBLon1;
        this.dBLon0 = dBLon0;
    }

    @Override
    public void store(String key, Product product) {
        dBLon0.store00("product", product);
    }
    
    @Override
    public Product load(String key) {
        return (Product)dBLon1.load00("product", key);         
    }

   
    @Override
    public Iterable<String> loadAllKeys() {
        return dBLon1.loadAllKeys00("product");
    }

    @Override
    public Map<String, Product> loadAll(Collection<String> keys) {
        Map<String, Product> m = new LinkedHashMap<>();
        keys.stream().forEach((String t) -> {
            m.put(t, load(t));
            
        });
        return m;
    }

}           
    
        